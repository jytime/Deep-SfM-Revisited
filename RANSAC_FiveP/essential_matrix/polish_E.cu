...
__host__ __device__ void Edecomp(Ematrix E, double parameters[5])
   ...
   // Fill in parameters
   parameters[0] = atan2(sx, cx);
   parameters[1] = atan2(sy, cy);
   parameters[2] = atan2(sz, cz);
   parameters[3] = atan2(su, cu);
   parameters[4] = atan2(sv, cv);
   }
...
//
//   // V -> V . Rv' . Ru'
//   Gright(V, 1, 2, e[4]);  // u - wrong index!
//   Gright(V, 0, 2, e[3]);  // v

   // DJC: Bug fix 20190409
   // U <- U * Rz * Ry * Rx
   Gright(U, 0, 1, e[2]);  // z
   Gright(U, 0, 2, e[1]);  // y
   Gright(U, 1, 2, e[0]);  // x
      ...
      for (int j=0; j<5; j++) // j is the point number
         {
          ...
          J[j][3] = -p[j][2] * q[j][1];                    // -pz*qx [DJC] -> -pz*qy
          J[j][4] = -p[j][2] * q[j][0];                    // -pz*qy [DJC] -> -pz*qx
         }
...
__host__ __device__ void polish_E (Ematrix E, const double* pin, const double* qin, const int MaxReps)
   ...
   for (int rep=0; ; rep++)
      ...
      for (int k=0; k<n; k++) { // Loop over the points
        ...
        JTerr[3] += -p[k][2] * q[k][1] * -errk; // -pz*qx [DJC] -> -pz*qy
        JTerr[4] += -p[k][2] * q[k][0] * -errk; // -pz*qx [DJC] -> -pz*qx
      }
      ...
      for (int k=0; k<n; k++) { // Loop over the points
        ...
        Jk[3] = -p[k][2] * q[k][1];                    // -pz*qx [DJC] -> -pz*qy
        Jk[4] = -p[k][2] * q[k][0];                    // -pz*qy [DJC] -> -pz*qx
        for (int i=0; i<5; i++) {
          for (int j=0; j<5; j++) {
            JTJ[i][j] += Jk[i] * Jk[j];
          }
...
__host__ __device__ void polish_E_huber (Ematrix E, double* pin, double* qin, const double delta, const int MaxReps)
   ...
   for (int rep=0; ; rep++)
      ...
      for (int k=0; k<n; k++) { // Loop over the points
        ...
        JTWerr[3] += -p[k][2] * q[k][1] * -errk * weights[k]; // -pz*qx [DJC] -> -pz*qy
        JTWerr[4] += -p[k][2] * q[k][0] * -errk * weights[k]; // -pz*qx [DJC] -> -pz*qx
      }
...
//   std::vector<std::array<double, 3>> q(n);
   ...
   for (int rep=0; ; rep++)
      ...
      for (int k=0; k<n; k++) { // Loop over the points
        ...
        Jk[2] =  p[k][1] * q[k][0] - p[k][0] * q[k][1];// py*qx - px*qy
        Jk[3] = -p[k][2] * q[k][1];                    // -pz*qx [DJC] -> -pz*qy
        Jk[4] = -p[k][2] * q[k][0];                    // -pz*qy [DJC] -> -pz*qy
        for (int i=0; i<5; i++) {
          for (int j=0; j<5; j++) {
            JTJ[i][j] += Jk[i] * Jk[j];
          }
...
//   std::vector<std::array<T, 3>> p(n);
//   std::vector<std::array<T, 3>> q(n);
