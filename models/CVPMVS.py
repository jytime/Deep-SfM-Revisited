...
class FeaturePyramid(nn.Module):
    ...
    def forward(self, img, scales=5):
        ...
        for scale in range(scales-1):
            img = nn.functional.interpolate(img,scale_factor=0.5,mode='bilinear',align_corners=None,recompute_scale_factor=True).detach()
            f = self.conv0aa(img)
            f = self.conv0bh(self.conv0bg(self.conv0bf(self.conv0be(self.conv0bd(self.conv0bc(self.conv0bb(self.conv0ba(f))))))))
            fp.append(f)
...
class CVPMVS(nn.Module):
    def __init__(self):
        super(CVPMVS, self).__init__()
    ...
    def forward(self, ref_img, src_imgs, P_mat,intrinsic,intrinsic_inv):
        # ref_in, src_in, ref_ex, src_ex, \
        #             depth_min, depth_max):

        # pre-process the input data to fit the raw setting of CVP-MVSNet

        ref_img = 0.5+ref_img/2
        src_imgs = src_imgs[0].unsqueeze(1)
        src_imgs = 0.5+src_imgs/2
        permute = [2, 1, 0]
        
        ref_img = ref_img[:,permute]
        src_imgs = src_imgs[:,:,permute]
        ...
        ## Feature extraction
        ref_feature_pyramid = self.featurePyramid(ref_img,self.nscale)

        src_feature_pyramids = []
        for i in range(self.nsrc):
            src_feature_pyramids.append(self.featurePyramid(src_imgs[:,i,:,:,:],self.nscale))
        ...
        depth_hypos = calSweepingDepthHypo(ref_in_multiscales[:,-1],depth_min, depth_max)
        ...
        for src_idx in range(self.nsrc):
            # warpped features
            # convert in and ex to rot and trans

            warped_volume = homo_warping(src_feature_pyramids[src_idx][-1], P_mat[:,src_idx], depth_hypos)
        ...
        # cost_volume = volume_sq_sum.div_(self.nsrc+1).sub_(volume_sum.div_(self.nsrc+1).pow_(2))
        cost_volume = volume_sq_sum
        ...
        for level in range(self.nscale-2,-1,-1):

            # Upsample
            depth_up = nn.functional.interpolate(depth[None,:],size=None,scale_factor=2,mode='bicubic',align_corners=None,recompute_scale_factor=True)
            depth_up = depth_up.squeeze(0)
            # Generate depth hypothesis
            depth_hypos = calDepthHypo(depth_up,level)
            
            cost_volume = proj_cost(ref_feature_pyramid[level],src_feature_pyramids,level,P_mat[:,src_idx],depth_hypos)
        ...
        return depth_est_list, depth_est_list[0].unsqueeze(1)
