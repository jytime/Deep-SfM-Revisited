...
class PWCDCNet(nn.Module):
    ...
    def __init__(self, md=4):
        ...
        # self.corr    = correlation.ModuleCorrelation()
    ...
    def forward(self,x1,x2):
        ...
        if self.training:
            #flow1 = F.interpolate(flow2, [im1.size()[2]/2,im1.size()[3]/2], mode='bilinear', align_corners=True)
            flow0 = F.interpolate(flow2, [im1.size()[2],im1.size()[3]], mode='bilinear', align_corners=True)
            return flow0 #, flow1, flow2, flow3, flow4
        else:
            return flow2 # we will interpolate it later, so it wil be fine without adjusting scales.
...
class PWCDCNet_old(nn.Module):
    ...
    def forward(self,x):
        ...
        if self.training:
            flow1 = F.interpolate(flow2, [im1.size()[2]/2,im1.size()[3]/2], mode='bilinear')
            flow0 = F.interpolate(flow2, [im1.size()[2],im1.size()[3]], mode='bilinear')
            return flow0, flow1, flow2, flow3, flow4
        else:
            return flow2
