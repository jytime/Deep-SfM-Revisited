...
def calSweepingDepthHypo(ref_in,depth_min, depth_max, nhypothesis_init=48):
    ...
    # for b in range(1,batchSize):depth_hypos = torch.cat((depth_hypos,torch.range(depth_min[0],depth_max[0],depth_interval_mean).unsqueeze(0)),0)
    depth_hypos = depth_hypos.expand(batchSize,-1)

    return depth_hypos.cuda()

def homo_warping(src_feature, proj, depth_hypos):
...
def calDepthHypo(ref_depths,level):
    ...
    depth_interval = torch.tensor([1]*nBatch).cuda() # Hard code the interval for training on DTU with 1 level of refinement.
...
def proj_cost(ref_feature,src_feature,level,proj,depth_hypos):
