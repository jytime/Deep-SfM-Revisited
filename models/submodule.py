...
class disparityregression(nn.Module):
    def __init__(self, maxdisp):
        ...
        self.disp = Variable(torch.Tensor(np.reshape(np.array(range(1,maxdisp+1)),[1,maxdisp,1,1])).cuda(), requires_grad=False)
        self.wsize = maxdisp//2
        self.pool3d = nn.MaxPool3d((2*self.wsize+1,1,1),stride=1,padding=(self.wsize,0,0))
    ...
    def forward(self, x):
        ...
        if cfg.TRUNC_SOFT:
            ...
            mask = mask.view(bat,1,c,h,w)
            mask = self.pool3d(mask)[0]
            ninf = x.clone().fill_(-np.inf).view(bat,c,h,w).detach()
            x = torch.where(mask.byte(),x,ninf)

        disp = self.disp.to(x.device)
        disp = disp.expand(x.size()[0],-1,x.size()[2],x.size()[3])
...
class depthregression(nn.Module):
    def __init__(self, maxdisp):
        ...
        depth_inter = int(cfg.MIN_DEPTH)
        max_depth = int(depth_inter*(maxdisp+1))
        self.disp = Variable(torch.Tensor(np.reshape(np.array(range(1*depth_inter,max_depth,depth_inter)),[1,maxdisp,1,1])).cuda(), requires_grad=False)
...
class disparityregression_lam(nn.Module):
    def __init__(self, maxdisp):
        super(disparityregression_lam, self).__init__()
        self.disp = Variable(torch.Tensor(np.reshape(np.array(range(-maxdisp,maxdisp+1)),[1,2*maxdisp+1,1,1])).cuda(), requires_grad=False)
                
    def forward(self, x):
        disp = self.disp.to(x.device).type_as(x)
        disp = disp.expand(x.size()[0],-1,x.size()[2],x.size()[3])
        out = torch.sum(x*disp,1)
        return out
